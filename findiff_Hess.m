function [Hessf] = findiff_Hess(f, x, h)
%
...
%
% OUTPUTS:
% Hessf = n-by-n matrix corresponding to the approximation of the Hessian 
% of f in x.


Hessf = zeros(length(x), length(x));
...
for j=1:length(x)
    ...
    Hessf(j,j) = (f(xh_plus) - 2*f(x) + f(xh_minus))/(h^2);
